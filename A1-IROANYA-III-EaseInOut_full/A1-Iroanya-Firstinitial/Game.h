...
public:
	...
	void update(float deltaTime) {
		...
		case Utils::SinusoidalEaseout:
			...
			break;
		case Utils::ParabolicEasein:
			i = Utils::lerp(0, pointsToTravel.size(), ParabolEaseIn(time / 5.0f));
			if (i >= pointsToTravel.size()) {
				return;
			}
			break;
		case Utils::ParabolicEaseout:
			i = Utils::lerp(0, pointsToTravel.size(), ParabolEaseOut(time / 5.0f));
			if (i >= pointsToTravel.size()) {
				return;
			}
			break;
