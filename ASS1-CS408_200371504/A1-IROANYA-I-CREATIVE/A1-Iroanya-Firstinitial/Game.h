...
public:
	...
	Game() {
		...
		size = 100;
		color = Color(0.25, 0.25, 0.25, 0.25);
	...
	void draw() {
		...
		//creative
		string msg = "\nThis is a special feature that gives a random color or size or shape depending on the user input. Press input once (this shuffles the options) and then again to accept the random output";
		string randomColorStr = "Random Color press 1: " + std::to_string(randomColor);
		string randomSizeStr = "Random Size press 2: " + std::to_string(randomSize);
		string randomShapeStr = "Random Shape press 3: " + std::to_string(randomShape);
		...
		//creative
		Shapes::drawText(5, 230, msg.c_str());
		Shapes::drawText(5, 270, randomColorStr.c_str());
		Shapes::drawText(5, 290, randomSizeStr.c_str());
		Shapes::drawText(5, 310, randomShapeStr.c_str());
	}
};